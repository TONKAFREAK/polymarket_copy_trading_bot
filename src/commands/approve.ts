...
export async function approveAllowances(): Promise<void> {
  ...
  console.log("This will approve the Polymarket Exchange contracts to trade on your behalf.");
  console.log("This is a ONE-TIME on-chain transaction (requires MATIC for gas).\n");
  ...
  const ctf = new ethers.Contract(POLYGON_CONTRACTS.conditionalTokens, ERC1155_ABI, wallet);
  ...
  const usdcAllowanceExchange: BigNumber = await usdc.allowance(wallet.address, POLYGON_CONTRACTS.exchange);
  const usdcAllowanceCTF: BigNumber = await usdc.allowance(wallet.address, POLYGON_CONTRACTS.conditionalTokens);
  const usdcAllowanceNegRisk: BigNumber = await usdc.allowance(wallet.address, POLYGON_CONTRACTS.negRiskExchange);
  const ctfApprovedExchange: boolean = await ctf.isApprovedForAll(wallet.address, POLYGON_CONTRACTS.exchange);
  const ctfApprovedNegRisk: boolean = await ctf.isApprovedForAll(wallet.address, POLYGON_CONTRACTS.negRiskExchange);

  console.log("Current allowances:");
  console.log(`  USDC → Exchange:     ${usdcAllowanceExchange.gt(0) ? "✅ Approved" : "❌ Not approved"}`);
  console.log(`  USDC → CTF:          ${usdcAllowanceCTF.gt(0) ? "✅ Approved" : "❌ Not approved"}`);
  console.log(`  USDC → NegRisk:      ${usdcAllowanceNegRisk.gt(0) ? "✅ Approved" : "❌ Not approved"}`);
  console.log(`  CTF → Exchange:      ${ctfApprovedExchange ? "✅ Approved" : "❌ Not approved"}`);
  console.log(`  CTF → NegRisk:       ${ctfApprovedNegRisk ? "✅ Approved" : "❌ Not approved"}`);
  console.log("");
  ...
  if (usdcAllowanceExchange.eq(0)) {
    ...
    const tx = await usdc.approve(POLYGON_CONTRACTS.exchange, ethers.constants.MaxUint256, gasSettings);
  ...
  if (usdcAllowanceCTF.eq(0)) {
    ...
    const tx = await usdc.approve(POLYGON_CONTRACTS.conditionalTokens, ethers.constants.MaxUint256, gasSettings);
    console.log(`   TX: ${tx.hash}`);
    await tx.wait();
    console.log("   ✅ Done\n");
    txCount++;
  ...
  if (usdcAllowanceNegRisk.eq(0)) {
    ...
    const tx = await usdc.approve(POLYGON_CONTRACTS.negRiskExchange, ethers.constants.MaxUint256, gasSettings);
    console.log(`   TX: ${tx.hash}`);
    await tx.wait();
    console.log("   ✅ Done\n");
    txCount++;
  ...
  if (!ctfApprovedExchange) {
    ...
    const tx = await ctf.setApprovalForAll(POLYGON_CONTRACTS.exchange, true, gasSettings);
    console.log(`   TX: ${tx.hash}`);
    await tx.wait();
    console.log("   ✅ Done\n");
    txCount++;
  ...
  if (!ctfApprovedNegRisk) {
    ...
    const tx = await ctf.setApprovalForAll(POLYGON_CONTRACTS.negRiskExchange, true, gasSettings);
    console.log(`   TX: ${tx.hash}`);
    await tx.wait();
    console.log("   ✅ Done\n");
    txCount++;
