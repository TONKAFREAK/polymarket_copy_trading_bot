...
export function createDiagnoseCommand(): Command {
  const command = new Command("diagnose")
    ...
    .action(async () => {
      ...
      console.log(
        `  ${chalk.cyan("Funder Address:")}      ${
          env.polyFunderAddress || "(not set)"
        }`
      );
      console.log(
        `  ${chalk.cyan("Signature Type:")}      ${env.polySignatureType} (${
          sigTypeNames[env.polySignatureType] || "Unknown"
        })`
      );

      // Check if wallets match
      const funderAddress =
        env.polyFunderAddress?.toLowerCase() || wallet.address.toLowerCase();
      ...
      if (walletMatch) {
        console.log(
          chalk.green(
            "  ✅ Wallet addresses match - configuration looks correct"
          )
        );
      } else {
        console.log(chalk.red("  ❌ WALLET MISMATCH DETECTED!"));
        console.log(
          chalk.yellow(`     Private key creates: ${wallet.address}`)
        );
        console.log(
          chalk.yellow(`     Funder address is:   ${env.polyFunderAddress}`)
        );
        console.log(
          chalk.yellow(
        ...
        console.log(
          chalk.yellow(
            "     Your funds are in the funder address, but the private key"
          )
        );
        ...
        console.log(
          chalk.gray("     1. Go to polymarket.com → Settings → Export Wallet")
        );
        console.log(
          chalk.gray(
            "     2. Replace PRIVATE_KEY in .env with the exported key"
          )
        );
      ...
      const walletBalanceFormatted = parseFloat(
      ...
      if (!walletMatch && env.polyFunderAddress) {
        ...
        const funderBalanceFormatted = parseFloat(
          ethers.utils.formatUnits(funderBalance, 6)
        );
        console.log(`  ${chalk.cyan("Funder Wallet:")}`);
        console.log(
          `    USDC: ${chalk.green(`$${funderBalanceFormatted.toFixed(2)}`)}`
        );

        if (funderBalanceFormatted > 0 && walletBalanceFormatted === 0) {
          console.log(
            chalk.yellow(
              `\n  ⚠️  Your $${funderBalanceFormatted.toFixed(
                2
              )} is in the funder wallet,`
            )
          );
          console.log(
            chalk.yellow(
              "     but your private key controls a different wallet!"
            )
          );
        }
      }
      ...
      console.log(
        `  ${chalk.cyan("MATIC (for gas):")} ${maticFormatted.toFixed(4)} MATIC`
      );

      // Check allowances for the appropriate wallet
      const checkAddress = walletMatch
        ? wallet.address
        : env.polyFunderAddress || wallet.address;
      ...
      const allowanceExchange = await usdc.allowance(
        checkAddress,
        CONTRACTS.exchange
      );
      const allowanceNegRisk = await usdc.allowance(
        checkAddress,
        CONTRACTS.negRiskExchange
      );
      const allowanceCTF = await usdc.allowance(
        checkAddress,
        CONTRACTS.conditionalTokens
      );
      const ctfApprovedExchange = await ctf.isApprovedForAll(
        checkAddress,
        CONTRACTS.exchange
      );
      const ctfApprovedNegRisk = await ctf.isApprovedForAll(
        checkAddress,
        CONTRACTS.negRiskExchange
      );

      const check = (approved: boolean) =>
        approved ? chalk.green("✅ Approved") : chalk.red("❌ Not approved");
      ...
      const allAllowancesOk =
        allowanceExchange.gt(0) &&
        allowanceNegRisk.gt(0) &&
        allowanceCTF.gt(0) &&
        ctfApprovedExchange &&
        ctfApprovedNegRisk;
      ...
      if (walletMatch && allAllowancesOk && walletBalanceFormatted > 0) {
        console.log(
          chalk.green("  ✅ Configuration looks good! You're ready to trade.")
        );
      } else {
        if (!walletMatch) {
          console.log(
            chalk.red("  ❌ Wallet mismatch - export Magic wallet private key")
          );
        }
        if (!allAllowancesOk) {
          console.log(
            chalk.yellow(
              "  ⚠️  Missing allowances - run: node dist/cli.js approve"
            )
          );
        }
        if (
          walletBalanceFormatted === 0 &&
          (walletMatch || !env.polyFunderAddress)
        ) {
          console.log(
            chalk.yellow("  ⚠️  No USDC balance - deposit funds to trade")
          );
        }
      }
