...
async function redeemPosition(
  ...
  try {
    ...
    console.log(`   📝 Redeeming position for condition ${conditionId.substring(0, 10)}...`);
...
export async function redeemWinnings(): Promise<void> {
  ...
  try {
    ...
    const ctf = new ethers.Contract(POLYGON_CONTRACTS.conditionalTokens, CTF_ABI, wallet);
    ...
    for (const pos of positions) {
      ...
      console.log(`${statusIcon} ${pos.outcome.toUpperCase().padEnd(4)} | ${sharesStr.padStart(10)} shares | ${valueStr.padStart(10)}`);
    ...
    if (redeemablePositions.length === 0) {
      ...
      for (const pos of redeemablePositions) {
        console.log(`Redeeming: ${pos.outcome} - ${pos.marketName.substring(0, 30)}...`);
        
        const result = await redeemPosition(wallet, ctf, pos.conditionId, gasSettings);
      ...
      console.log(`\n${"─".repeat(60)}`);
      console.log(`Redeemed: ${totalRedeemed}/${redeemablePositions.length} positions`);
      console.log(`💰 USDC Before: ${ethers.utils.formatUnits(usdcBefore, 6)} USDC`);
      console.log(`💰 USDC After:  ${ethers.utils.formatUnits(usdcAfter, 6)} USDC`);
      if (usdcGained.gt(0)) {
        console.log(`💵 Gained:      +${ethers.utils.formatUnits(usdcGained, 6)} USDC`);
      }
      console.log("");
    }
...
export async function redeemByConditionId(conditionId: string): Promise<void> {
  ...
  // Check if resolved
  const { isResolved, winningOutcome } = await checkConditionResolution(ctf, conditionId);
  ...
  if (result.success) {
    ...
    console.log("  - The market uses NegRisk (requires different redemption)\n");
  }
}
...
export async function redeemByTokenId(tokenId: string): Promise<{
  ...
  try {
    ...
    const marketName = String(marketInfo.question || marketInfo.title || "Unknown");
    ...
    if (result.success) {
      // Check USDC balance after
      const usdcAfter = await usdc.balanceOf(wallet.address);
      const usdcGained = parseFloat(ethers.utils.formatUnits(usdcAfter.sub(usdcBefore), 6));
